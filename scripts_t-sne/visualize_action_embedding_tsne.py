...
AGGREGATION_METHODS = ["mean", "max", "last", "first_last", "delta_total", "delta_mean"]
...
def _load_policy_with_vision_lora(ckpt: Path, device: str):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if policy_type == "groot_cl":
            ...
            config = draccus.parse(GrootCLv2Config, tmp_cfg, args=[])
        else:
    ...
    if lora_rank > 0 and lora_target in ("llm", "both"):
...
def load_model_and_preprocessor(checkpoint_dir: str, dataset_stats: dict, device: str):
    ...
    if policy_type in ("groot_cl", "groot_cl_v2") or (lora_rank > 0 and lora_target in ("vision", "both")):
