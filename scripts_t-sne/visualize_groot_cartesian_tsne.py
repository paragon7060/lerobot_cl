#!/usr/bin/env python3
"""
t-SNE / interactive visualization for GROOT cartesian model.
INSIGHTfixposV3_EE: axis-angle action space (7D), 3-camera setup.

Features:
  - Backbone + action_encoder dual t-SNE (side-by-side)
  - Per-task coloring with group toggle buttons
  - Click a point → video clip playback (A/B comparison)
  - A/B feature distance panel (cosine + L2, full + PCA)

Usage:
    cd ~/ws3/lerobot
    conda run -n lerobot050_groot python -u \
        src/lerobot/scripts/visualize_groot_cartesian_tsne.py \
        --checkpoint_dir /path/to/pretrained_model \
        --dataset_repo_id Whalswp/INSIGHTfixposV3_EE \
        --samples_per_task 32 \
        --batch_size 16 \
        --task_prompt_mode non_guide \
        --output_dir ./outputs/tsne_cartesian
"""
...
DEFAULT_CAM_KEYS = ["wrist", "right_shoulder", "guide"]
...
def parse_args():
    ...
    p.add_argument("--dataset_repo_id",  type=str, default="Whalswp/INSIGHTfixposV3_EE")
    p.add_argument("--dataset_root",     type=str, default=None,
                   help="Local dataset root (None = HF cache)")
    ...
    p.add_argument("--output_dir",       type=str, default="./outputs/tsne_cartesian")
    ...
    p.add_argument("--video_fps",        type=float, default=None,
                   help="Video clip FPS. None이면 ds_meta.fps 사용 (권장).")
...
def load_dataset(repo_id, root, delta_timestamps=None, pop_keys=None):
    from lerobot.datasets.lerobot_dataset import LeRobotDataset
    kw = dict(repo_id=repo_id, root=root, video_backend="pyav")
    if delta_timestamps:
...
@torch.no_grad()
def extract_features(policy, preprocessor, dataset, manifest,
                     task_name_to_desc, action_dim, state_dim,
                     batch_size, device):
    ...
    for start in range(0, N, batch_size):
        ...
        # Slice dims (cartesian: action 7D, state 16D)
...
def extract_video_clips(manifest: list[dict], cam_keys: list[str],
    ...
    for si, m in enumerate(manifest):
        for ck in cam_keys:
            ...
            if vpath is None:
                if si == 0:
                    print(f"  [clips] WARNING: no video for cam='{ck}' ep={m['episode_index']} — skipping")
                continue
            ...
            with av.open(buf, mode="w", format="mp4") as oc:
                ...
                _rate = _Frac(fps).limit_denominator(1000) if isinstance(fps, float) else fps
                ost = oc.add_stream("libx264", rate=_rate)
...
def plot_overview(X2d_a, X2d_b, groups, task_names, descriptions,
    ...
    fig.suptitle(f"Cartesian t-SNE  perp={perplexity}  N={len(groups)}", fontsize=13)
    fig.tight_layout()
...
<div class="cam-group">
  ...
  <div class="cam-row">
    ...
    title = (f"Cartesian t-SNE  |  action ({action_agg}) + backbone ({backbone_agg})"
...
def main():
    ...
    # ── Task descriptions ─────────────────────────────────────────────────────
    print("[0] Loading task descriptions ...")
    from lerobot.datasets.dataset_metadata import LeRobotDatasetMetadata

    ds_meta = LeRobotDatasetMetadata(
        repo_id=args.dataset_repo_id, root=args.dataset_root)

    # info.json shape가 32D로 남아 있으면 make_policy가 잘못된 크기로 구성됨 → 패치
    if "action" in ds_meta.features:
        ds_meta.features["action"]["shape"] = (args.action_dim,)
    if "observation.state" in ds_meta.features:
        ds_meta.features["observation.state"]["shape"] = (args.state_dim,)
    ...
    print("  description map:")
    for task_name in sorted(task_name_to_desc):
        print(f"    {task_name} -> {task_name_to_desc[task_name]}")
    ...
    if cache_path.exists() and not args.skip_feature_cache:
        ...
        dataset_plain = load_dataset(args.dataset_repo_id, args.dataset_root)
        ...
        # delta_timestamps: 학습과 동일하게 policy.config 기반으로 전체 키에 대해 빌드
        # (observation history 등 누락 방지). 그 위에 action horizon만 덮어쓴다.
        delta_ts = resolve_delta_timestamps(policy.config, ds_meta) or {}
        delta_ts = dict(delta_ts)  # 원본 변형 방지
        delta_ts["action"] = [i / ds_meta.fps for i in range(args.action_horizon)]
        print(f"  delta_timestamps keys: {sorted(delta_ts.keys())}")
        dataset = load_dataset(args.dataset_repo_id, args.dataset_root, delta_ts)
    ...
    # ── Video clips ───────────────────────────────────────────────────────────
    clips_dirs = {}
    clip_dataset_root = Path(args.dataset_root) if args.dataset_root else ds_meta.root
    if not args.skip_clips and args.cam_keys:
        print(f"\n[5] Extracting video clips (cams={args.cam_keys}) ...")
        print(f"  clip dataset root: {clip_dataset_root}")
        ep_video_meta = load_episode_video_meta(clip_dataset_root)
        print(f"  episode video meta: {len(ep_video_meta)} episodes")
        W, H = map(int, args.clip_size.split("x"))
        clip_fps = float(args.video_fps) if args.video_fps is not None else float(ds_meta.fps)
        print(f"  clip fps: {clip_fps} (source: {'--video_fps' if args.video_fps is not None else 'ds_meta.fps'})")
        clips_dirs = extract_video_clips(
            manifest, args.cam_keys, args.action_horizon,
            out_dir / "clips", fps=clip_fps,
            dataset_root=clip_dataset_root,
            output_size=(W, H),
            ep_video_meta=ep_video_meta)
        clips_dirs = {ck: d for ck, d in clips_dirs.items()
                      if d.exists() and any(d.glob("*.mp4"))}
        if clips_dirs:
            print(f"  available cams: {list(clips_dirs.keys())}")
        else:
            print("  WARNING: no clips generated")
