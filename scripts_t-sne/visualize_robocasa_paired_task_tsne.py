#!/usr/bin/env python3
"""
t-SNE visualization for RoboCasa atomic task PAIRS that share the same initial
observation but differ only in task instruction (task_key or description).
...
Example:
  ssh seonho@166.104.35.48 "cd ~/ws3/lerobot && \\
      /home/seonho/miniconda3/envs/lerobot050_groot/bin/python \\
          -m lerobot.scripts.visualize_paired_task_tsne \\
          ...
          --samples_per_desc 16 \\
          --cache_dir ~/ws3/outputs/tsne_vis_paired"
"""
...
def parse_args():
    ...
    p.add_argument("--cache_dir", type=str, default="./outputs/tsne_vis_paired")
    ...
    p.add_argument("--rp_dim", type=int, default=128,
    ...
    p.add_argument("--no_proc", action="store_true",
                   help="Skip process_backbone_output (vlln+vl_self_attention) and toggle button")
...
def extract_features(policy, preprocessor, manifest: list[dict],
    ...
    for start in range(0, N_total, batch_size):
        ...
        if with_proc:
            ...
            with torch.autocast(device_type=dev_type, dtype=torch.bfloat16, enabled=use_bf16):
                proc_out = action_head.process_backbone_output(bb_out)
            bb_proc_f_list.append(proc_out["backbone_features"].detach().float().cpu())
            bb_proc_m_list.append(proc_out["backbone_attention_mask"].detach().cpu())
    ...
    return action_feats, gt_actions_np, bb_f_list, bb_m_list, bb_proc_f_list, bb_proc_m_list, meta
...
def save_cache(path: Path, action_feats, gt_actions,
               bb_f_list, bb_m_list, bb_proc_f_list, bb_proc_m_list, meta):
    def _pack(lst):
...
def load_cache(path: Path):
    ...
    bb_proc_f = ([torch.from_numpy(x.astype(np.float32)) for x in npz["bb_proc_feats"]]
                 if "bb_proc_feats" in npz else [])
    bb_proc_m = ([torch.from_numpy(x) for x in npz["bb_proc_masks"]]
                 if "bb_proc_masks" in npz else [])
    meta = {
    ...
    return npz["action_feats"], npz["gt_actions"], bb_f, bb_m, bb_proc_f, bb_proc_m, meta
...
<div class="cam-group">
  ...
  <div class="cam-row">
    ...
    backbone_xy_alt  = X2d_backbone_alt.tolist() if X2d_backbone_alt is not None else None
    has_bb_alt       = backbone_xy_alt is not None
    ...
    title = (f"Paired Task t-SNE  |  action_encoder ({action_agg}) vs backbone ({backbone_agg})"
...
function sampleInfo(idx) {{ return '<b>'+groupLabels[idx]+'</b>\\n'+descLabels[idx]+'\\nsample: '+idx+'\\nframe: '+frameIdxs[idx]; }}
...
def main():
    ...
    if cache_path.exists() and not args.skip_feature_cache:
        ...
        action_feats, gt_actions, bb_f_list, bb_m_list, bb_proc_f_list, bb_proc_m_list, meta = load_cache(cache_path)
        ...
        action_feats, gt_actions, bb_f_list, bb_m_list, bb_proc_f_list, bb_proc_m_list, meta = extract_features(
    ...
    if bb_proc_f_list:
        ...
        _, X2d_bb_proc = run_tsne(X_backbone_proc, args.perplexity, args.tsne_n_iter, args.seed,
                                  "backbone_processed", zscore=zscore)
