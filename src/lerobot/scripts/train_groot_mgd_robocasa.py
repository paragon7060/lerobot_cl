...
def main(cfg: GrootMGDTrainConfig) -> None:
    ...
    if accelerator.is_main_process:
        ...
        other_trainable = sum(
            ...
            if p.requires_grad and not n.startswith(selected_prefixes)
        )
        ...
        if use_wandb:
            ...
            wandb.summary["params/vlln_total"] = int(vlln_total)
            ...
            wandb.summary["params/selected_trainable"] = int(selected_train)
            wandb.summary["params/other_trainable"] = int(other_trainable)
            wandb.summary["params/sequence_over_processed_ratio"] = (
                float(seq_train) / float(vlln_train + vlsa_train)
                if (vlln_train + vlsa_train) > 0
                else 0.0
            )
            wandb.summary["params/selected_over_whole_ratio"] = (
                float(selected_train) / float(num_total_params) if num_total_params > 0 else 0.0
            )
